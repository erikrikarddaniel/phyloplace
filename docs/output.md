...
- `hmmer/`
  ...
  - `*.hmmrank.tsv.gz`: Summarised `hmmsearch` results, one row per sequence and profile, ranking the profiles that matched each sequence.
    ...
    `qlen` is the profile's own length, so `hmm_len / qlen` is the proportion of the profile a hit covers, and `hmm_from - 1` and `qlen - hmm_to` are how much is missing from each of its ends.
